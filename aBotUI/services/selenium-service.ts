...
interface IPaths {
    dirPath: string;
    fileName: string;
    reportFileDirPath: string;
    screenshotDirPath: string;
    screenshotLiveDirPath: string;
    wdioConfigPath: string;
    missionWdioConfigPath: string;
    errorShotDirPath: string;
}
...
export class Service {
    ...
    private cleanProcess() {
        return new Bluebird((resolve, reject) => {
            PS.lookup({ command: ".selenium" }, (err, infos) => {
                ...
                const geckodriverPidList = [] as number[];
                ...
                if (hasSeleniumServer === false && geckodriverPidList.length > 0) {
                    ...
                    while (geckodriverPidList.length > 0) {
                        const pid = geckodriverPidList.pop() as number;
    ...
    private getPaths(currentMission: IMissionModel, currentTask: ITaskModel, seq: number) {
        const result = {} as IPaths;
    ...
    private generateWdioConfigFile(currentMission: IMissionModel, currentTask: ITaskModel, seq: number) {
        return new Bluebird<void>((resolve, reject) => {
            const paths = this.getPaths(currentMission, currentTask, seq);
